...
Standard POSIX tools (`git`, `grep`, `sed`, `find`, etc.) are available through
`bash`. There is no Node, no package manager beyond what the repo provides, and
no network egress beyond what the runner allows.
